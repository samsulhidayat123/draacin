...
export default async function FilmIndoDetail({ params }) {
  const { slug } = await params;
  const item = await getContentBySlugAndType(slug, "film_indo").catch(() => null);

  if (!item) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-black p-6 text-center text-white">
        <h1 className="mb-3 text-3xl font-black uppercase">Film Indo tidak ditemukan</h1>
        <p className="mb-8 max-w-md text-sm text-gray-400">
          Data belum tersedia dari MongoDB Atlas.
        </p>
        <Link
          href="/?type=film_indo"
  ...
  return <ContentDetail item={item} expectedType="film_indo" />;
}
