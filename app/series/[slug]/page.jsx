...
export default async function SeriesDetail({ params }) {
  ...
  const item = await getContentBySlugAndType(slug, "series").catch((error) => {
    console.error("Fetch series from MongoDB error:", error.message);
    return null;
  });
  ...
  if (!item) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-black p-6 text-center text-white">
        <h1 className="mb-3 text-3xl font-black uppercase">Series tidak ditemukan</h1>
        <p className="mb-8 max-w-md text-sm text-gray-400">
          Data series belum tersedia dari MongoDB Atlas.
        </p>
        <Link
          href="/"
  ...
  return <ContentDetail item={item} expectedType="series" />;
}
