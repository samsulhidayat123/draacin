...
export default async function NetflixDetail({ params }) {
  const { slug } = await params;
  const item = await getContentBySlugAndType(slug, "netflix").catch(() => null);

  if (!item) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-black p-6 text-center text-white">
        <h1 className="mb-3 text-3xl font-black uppercase">Netflix series tidak ditemukan</h1>
  ...
  return <ContentDetail item={item} expectedType="netflix" />;
}
